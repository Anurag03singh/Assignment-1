    ...
    <div className="p-6">
      ...
      <div className="space-y-4">
        {repositories.map((repo) => (
          <div key={repo.name} className="flex items-center justify-between p-4 bg-white rounded-lg border hover:shadow-md transition-shadow">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="font-medium">{repo.name}</span>
                <span className={`px-2 py-0.5 rounded-full text-xs ${
                  repo.isPrivate ? 'bg-gray-100 text-gray-700' : 'bg-blue-100 text-blue-700'
                }`}>
